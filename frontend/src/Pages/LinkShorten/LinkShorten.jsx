...
import { createNewLinkURL, verifyURLFromDBlink } from '../../api/linkFunctions'
...
function LinkShorten() {
  ...
  const [urlToShrinkError, setUrlToShrinkError] = useState(false)
  const [urlToShrinkErrorMsg, setUrlToShrinkErrorMsg] = useState("")
  ...
  const submit = async () => {
    try {
      ...
      if (urlToShrink.trim().length == 0) {
        setUrlToShrinkError(true)
        setUrlToShrinkErrorMsg("Enter the Website URL")
        return
      }
      else if (!isUrl(urlToShrink)) {
        console.log(isUrl(urlToShrink))
        setUrlToShrinkError(true)
        setUrlToShrinkErrorMsg('Enter a valid URL.');
        return;
      }
      else {
      ...
      const { url, status } = await createNewLinkURL(dataToSend)
      if (status) {
  ...
  //     submit(true)
  //   }
  //   const closeModalCancel = () => modal.hide()
  //   const showModal = () => modal.show()


  //   if (action == "show") showModal()
  //   else if (action == "continue") closeModalContinue()
  //   else closeModalCancel()
  // }
  ...
  const verifyURL = async () => {
    ...
    if (URL.trim().length == 0) {
      ...
      const { verified } = await verifyURLFromDBlink(URL)
      if (!verified) {
        seturlErrorMsg("URL already taken")
        setURL("")
        seturlError(true);
  ...
  return (
    <div className="bg-[#f9f9f9]">
      ...
      <div className="mx-3 rounded-md sm:mx-auto max-w-xl py-3 sm:px-6 sm:py-5 lg:px-8 flex  mt-[4rem] sm:mt-20 items-center justify-center bg-white" style={{ boxShadow: '0 1px 4px #ccc' }}>
        ...
        <form className='min-w-[70%] w-[min-content]'>
          <div className="mb-7">
            <label htmlFor="website" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Website URL</label>
            <input type="url" id="website" value={urlToShrink} onChange={(e) => {
              setUrlToShrink(e.target.value)
              if(urlToShrinkError) setUrlToShrinkError(false)
            }} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="shorturlz.in" required />
            <div className={urlToShrinkError ? "" : "hidden"}>
              {/* <p className={"mt-2 text-sm " + (urlToShrinkError ? "text-red-600 dark:text-red-500" : "hidden")}><span className="font-medium">Oh, snapp!</span> {urlToShrinkErrorMsg}</p> */}
              <div className="flex items-center mt-2 p-2 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                <svg className="flex-shrink-0 inline w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z" />
                </svg>
                <span className="sr-only">Info</span>
                <div>
                  <span className="font-medium">Oops!</span> {urlToShrinkErrorMsg}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className={customURL == true ? "mb-7" : "mb-7 hidden"}>
            ...
            <div className={urlError ? "" : "hidden"}>
              <div className="flex items-center mt-2 p-2 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                ...
                <span className="sr-only">Info</span>
                <div>
                  <span className="font-medium">Oops!</span> {urlErrorMsg}
                </div>
              </div>
            </div>
          ...
          <div className={passwordProtection == true ? "" : "hidden"}>
            ...
            <div className={passwordError ? "" : "hidden"}>
              <div className="flex items-center mt-1 p-2 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                <svg className="flex-shrink-0 inline w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z" />
                </svg>
                <span className="sr-only">Info</span>
                <div>
                  <span className="font-medium">Oops!</span> {passwordErrorMsg}
                </div>
              </div>
            </div>
          </div>
      ...
      {/* <div id="popup-modal" tabindex="-1" className="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div className="relative p-4 w-full max-w-md max-h-full">
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button type="button" onClick={() => warningModal("cancel")} className="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="popup-modal">
            ...
            <div className="p-4 md:p-5 text-center">
              ...
              <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">The Website link is not reachable. Do you want to continue?</h3>
              <button data-modal-hide="popup-modal" onClick={() => warningModal("continue")} type="button" className="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                Yes, Continue
              </button>
              <button data-modal-hide="popup-modal" onClick={() => warningModal("cancel")} type="button" className="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">No, cancel</button>
            </div>
          </div>
        </div>
      </div> */}

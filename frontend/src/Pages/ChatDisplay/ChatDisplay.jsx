...
function ChatDisplay() {
    ...
    useEffect(() => {
        async function fetchData() {
            const response = await initialChatCheck(chatId);
    ...
    const authenticate = async () => {
        if (password.trim().length == 0) {
            ...
            const { status, message } = await fetchProtectedChat(chatId, password)
            if (status) {
                setMessageHistory(message)
                ...
                if (tokens && tokens[chatId]) {
                    let userData = tokens[chatId]
    ...
    const sendMessage = async () => {
        if (newMessage.trim().length == 0) { }
            ...
            const { status } = await sendNewMessage(chatId, userId, newMessage)
            if (status) {
                setNewMessage("")
            }
        }
    ...
    return (
        <>
            {isLoadingPage ? <LoadingScreen /> : isValid ? <div className="bg-white">
                ...
                <div className="relative isolate px-6 pt-1 lg:px-8 mt-24 sm:mt-0">
                    ...
                    <div className="mx-auto max-w-5xl sm:w-4/5 w-full  py-2 sm:px-6 sm:py-32 lg:px-8 flex items-center justify-center">
                        <div className="relative isolate overflow-hidden bg-white w-full shadow-2xl rounded-2xl sm:rounded-3xl   lg:pt-0">
                            <div className="w-full h-14" style={{ backgroundColor: "#2d9afa" }}>
                                dfs
                            </div>
                            <div className="mx-auto h-64 sm:h-48 md:h-64 lg:h-96 xl:h-128 sm:px-10 lg:mx-0 lg:flex-auto lg:py-2 ">
                                {messageHistory.length == 0 ? ( <div>No Messages to Show</div> ) : (messageHistory.map((item) => (
                                    item.senderId == userId ? <div className="chat chat-end">
                            ...
                            <div className="w-full h-14 flex pr-4 pl-4 pb-4">
                                <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} id="success" class="bg-green-50 border border-green-500 text-green-900 dark:text-green-400 placeholder-green-700 dark:placeholder-green-500 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 dark:bg-gray-700 dark:border-green-500 mr-4" placeholder="Success input" />
                                <button type="button" onClick={sendMessage} class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center me-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                    <svg class="w-5 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
                ...
                <div className="modal-box">
                    ...
                    <div className='flex moal-action justify-center mt-5'>
                        <div class="relative mb-1 w-2/3">
                            ...
                            }} id="input-group-1" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Password" />
                        </div>
                    </div>
                    <div class={passwordError ? "justify-center flex" : "justify-center flex hidden"}>
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">Oh, snapp! </span>{passwordErrorMsg}</p>
                    </div>

                    <div className="modal-action">
                        {/* <form method="dialog"> */}
                        <button onClick={authenticate} class="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-pink-500 to-orange-400 group-hover:from-pink-500 group-hover:to-orange-400 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-pink-200 dark:focus:ring-pink-800">
            ...
            <dialog id="nameModal" className="modal">
                <div className="modal-box">
                    ...
                    <div className="modal-action">
                        {/* <form method="dialog"> */}
                        <button onClick={setUsernameFunction} class="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-pink-500 to-orange-400 group-hover:from-pink-500 group-hover:to-orange-400 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-pink-200 dark:focus:ring-pink-800">
                            <span class="relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0">
                                Submit
                            </span>
                        </button>

...
import './Note.css'
import { createNewNoteURL, verifyURLFromDBNote } from '../../api/NoteFunctions'
...
function Note() {
  ...
  const submit = async () => {
    try {
      ...
      if (textToSave.trim().length == 0) {
        setTextErr(true)
        setTextErrMsg("Enter the Data to save")
        return
      }
      else {
  ...
  const verifyURL = async () => {
    ...
    if (URL.trim().length == 0) {
      ...
      const { verified } = await verifyURLFromDBNote(URL)
      if (!verified) {
        seturlErrorMsg("URL already taken")
        setURL("")
        seturlError(true);
  ...
  return (
    <div className="bg-[#f9f9f9]">
      ...
      <div className="relative isolate px-6 pt-1 lg:px-8 mt-8 sm:mt-0">
        <div
        ...
        <div className="mx-auto max-w-7xl py-2 sm:px-6 sm:pt-32 lg:px-8 flex items-center justify-center">
          <div className="relative isolate overflow-hidden bg-gray-900 px-2 pt-1 w-full shadow-2xl rounded-2xl sm:rounded-3xl sm:px-5 md:pt-2 lg:flex  lg:px-8 lg:pt-0">
            ...
            <div className="mx-auto text-center lg:mx-0 lg:flex-auto lg:py-2 lg:text-left">
              <textarea value={textToSave} id='noteInput' onChange={(e) => setTextToSave(e.target.value)} className="h-64 sm:h-48 md:h-64 lg:h-96 xl:h-128 w-full textarea bg-transparent text-white leading-snug resize-none " placeholder="Enter the data here...."></textarea>
            </div>
          </div>

        </div>
        <div className={textErr ? "mx-auto max-w-7xl py-2 sm:px-6  lg:px-8" : "mx-auto max-w-7xl py-2 sm:px-6 sm:py-32 lg:px-8 hidden"}>
          <div className="flex items-center mt-2 p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
            <svg className="flex-shrink-0 inline w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
              <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z" />
            </svg>
            <span className="sr-only">Info</span>
            <div>
              <span className="font-medium">Oops!</span> {textErrMsg}
            </div>
          </div>
        </div>

        <div className="mx-3 rounded-md sm:mx-auto max-w-xl px-3 py-3 sm:px-6 sm:py-5 lg:px-8 flex  mt-[4rem] sm:mt-20 items-center justify-center bg-white" style={{ boxShadow: '0 1px 4px #ccc' }}>
          ...
          <div className='modal-action justify-center'>
            <div className="w-full max-w-[20rem]">
              <div className="relative ">
                ...
                <input id="copy-text" value={modalURL} type="text" className="col-span-6 bg-gray-50 border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-2.5 py-4 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" disabled readOnly />
                <button data-copy-to-clipboard-target="npm-install-copy-text" onClick={copyToClipboard} id='copy-clipboard-button' className="absolute end-2.5 top-1/2 -translate-y-1/2 text-gray-900 dark:text-gray-400 hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-600 dark:hover:bg-gray-700 rounded-lg py-2 px-2.5 inline-flex items-center justify-center bg-white border-gray-200 border">
